...
wget -nc https://www.dovecot.org/releases/2.3/dovecot-2.3.9.2.tar.gz


NAME=dovecot
VERSION=2.3.9.2
URL=https://www.dovecot.org/releases/2.3/dovecot-2.3.9.2.tar.gz
...
./configure --prefix=/usr                          \
            --sysconfdir=/etc                      \
            --localstatedir=/var                   \
            --docdir=/usr/share/doc/dovecot-2.3.9.2 \
            --disable-static                       \
...
sudo rm -rf /tmp/rootscript.sh
cat > /tmp/rootscript.sh <<"ENDOFROOTSCRIPT"
cp -rv /usr/share/doc/dovecot-2.3.9.2/example-config/* /etc/dovecot
ENDOFROOTSCRIPT
